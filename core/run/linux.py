...
def stop():
    password = ctk.CTkInputDialog(text="Because you're using Linux, you need to enter your password to continue. We promise not to send your password to us!", title="Enter password").get_input()
    if password is not None:
        execute = sb.Popen(['sudo', '-S', 'shutdown', '-c'],
            stdin=sb.PIPE, 
            stdout=sb.PIPE, 
            stderr=sb.PIPE,
            shell=False,
...
def start(time, action, comment):
    ...
    if password is not None:
        if comment == '':
            if action =='Shut down':
                execute = sb.Popen(['sudo', '-S', 'shutdown', '+'+str(time)],
                        stdin=sb.PIPE, 
                        stdout=sb.PIPE, 
                        stderr=sb.PIPE,
                        text=True)
            ...
            if action =='Shut down':
                execute = sb.Popen(['sudo', '-S', 'shutdown', '+'+str(time), comment],
                        stdin=sb.PIPE, 
                        stdout=sb.PIPE, 
                        stderr=sb.PIPE,
                        text=True,)
